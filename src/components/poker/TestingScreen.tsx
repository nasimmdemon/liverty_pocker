...
/* ── Animation Preview Helpers ── */

const DEMO_CARD: CardType = { suit: '♠', rank: 'A', faceUp: true };
const DEMO_CARD_BACK: CardType = { suit: '♥', rank: 'K', faceUp: false };
  ...
  if (!card.faceUp) {
    return (
      <motion.div
        ...
        style={{
          backgroundImage: `url(/card_bg_1.png)`,
...
const ANIM_LIST: AnimItem[] = [
  { id: 'chip-to-pot', label: 'Chip → Pot', description: 'Chips flying from player to pot' },
  { id: 'win-chips', label: 'Win Chips', description: 'Chips showering to winner' },
  { id: 'card-deal', label: 'Card Deal', description: 'Cards spinning in with 3D effect' },
  { id: 'card-flip', label: 'Card Flip', description: 'Card revealing face-up' },
  ...
  return (
    ...
    >
      <AnimatePresence mode="wait">
        <motion.div key={`${id}-${cycle}`} className="flex items-center justify-center gap-4">
  ...
  return (
    <div className="relative w-full h-full flex items-center justify-center">
      {[0, 1, 2].map(i => (
        ...
        >
          <ChipIcon size={36} variant="red" />
        </motion.div>
      ))}
      <motion.span
        className="text-primary font-bold text-sm absolute -top-2"
  ...
  return (
    <div className="relative w-full h-full flex items-center justify-center">
      {Array.from({ length: chipCount }).map((_, i) => (
        ...
        >
          <ChipIcon size={24} variant="red" />
        </motion.div>
      ))}
      <motion.span
        className="absolute text-primary font-bold text-base drop-shadow-lg"
        style={{ fontFamily: "'Bebas Neue', sans-serif" }}
...
function CardDealPreview() {
  const cards: CardType[] = [
    ...
    { suit: '♥', rank: 'K', faceUp: false },
    { suit: '♦', rank: 'Q', faceUp: false },
    { suit: '♣', rank: 'J', faceUp: false },
  ];
  return (
    <div className="flex gap-2">
      {cards.map((c, i) => <MiniCard key={i} card={c} delay={i * 0.2} />)}
    </div>
  );
}
...
function CardFlipPreview() {
  ...
  return (
    <div className="flex gap-3">
      {cards.map((c, i) => <MiniCard key={i} card={c} delay={i * 0.3} />)}
    </div>
  );
}
...
function TimerRingPreview() {
  return (
    <div className="relative w-24 h-24">
      ...
      <motion.span
        className="absolute inset-0 flex items-center justify-center text-primary font-bold text-lg"
        style={{ fontFamily: "'Bebas Neue', sans-serif" }}
        initial={{ opacity: 1 }}
        animate={{ opacity: [1, 1, 0.5, 1] }}
        transition={{ duration: 1, repeat: 9 }}
      >
        <motion.span
          initial={{ scale: 1 }}
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 w-[90%] max-w-sm mt-4 flex gap-2" style={{ fontFamily: "'Bebas Neue', sans-serif" }}>
        ...
        <button className={tabClass('animations')} onClick={() => setActiveTab('animations')}>
          🎬 ANIMATIONS
        </button>
      </div>
      ...
      <AnimatePresence mode="wait">
        {activeTab === 'config' ? (
          <motion.div
            key="config"
            className="relative z-10 w-[90%] max-w-sm mt-4 flex flex-col gap-4"
          ...
          <motion.div
            key="animations"
            className="relative z-10 w-[90%] max-w-sm mt-4 flex flex-col gap-3 pb-24"
            initial={{ opacity: 0, x: 20 }}
            animate={{ opacity: 1, x: 0 }}
            exit={{ opacity: 0, x: 20 }}
            transition={{ duration: 0.2 }}
          >
            ...
            <div className="flex items-center justify-between px-1">
              <span className="text-xs text-muted-foreground tracking-wider" style={{ fontFamily: "'Bebas Neue', sans-serif" }}>
                PREVIEW ANIMATIONS
              </span>
              <button
                onClick={() => setLoopAnim(!loopAnim)}
                className={`flex items-center gap-1.5 text-xs px-3 py-1 rounded-full border transition-colors ${
                  loopAnim ? 'border-primary/60 text-primary bg-primary/10' : 'border-muted/40 text-muted-foreground'
                }`}
              >
                <RotateCcw size={12} /> {loopAnim ? 'Loop ON' : 'Loop OFF'}
              </button>
            </div>
                ...
                <div key={anim.id} className="rounded-xl overflow-hidden" style={{
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <span className="text-sm font-medium text-foreground block" style={{ fontFamily: "'Bebas Neue', sans-serif", letterSpacing: '0.08em' }}>
                        {anim.label}
                      </span>
